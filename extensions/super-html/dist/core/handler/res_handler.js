...
class res_handler {
    ...
    create_html() {
        ...
        config_1.default.s_out_dir = config_1.default.s_out_dir.replace("super-html", config_1.default.d_hot.s_title);
        //替换css
    ...
    create_res_cache() {
        ...
        for (let i = 0; i < l_path.length; i++) {
            ...
            if (s_file_path.endsWith(".wasm")) {
                ...
                s_err += "\n zh : 发现wasm格式文件, 不支持此类型，请升级引擎3.8.3以上版本，构建选择AsmJS";
                s_err += "\n en : Discovered a WASM format file, unsupported type, please upgrade to engine version 3.8.3 or above, and select AsmJS for building.";
                s_err += "\n---- super-html error ----";
                console.error(s_err);
